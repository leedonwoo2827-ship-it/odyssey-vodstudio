...
if AUTH_ENABLED:
    AUTH_EXEMPT_EXACT = {
        ...
        "/api/auth/login",
        "/api/auth/logout",
        # Google OAuth endpoints must be reachable before a session exists.
...
# 영상공방 (VOD Studio) — NotebookLM 슬라이드/대본 → mp4maker 번들
from routes.vodstudio_routes import setup_vodstudio_routes
app.include_router(setup_vodstudio_routes())
...
@app.get("/vodstudio")
async def serve_vodstudio(request: Request):
    """영상공방 스튜디오 페이지 (NotebookLM → 슬라이드/대본 → mp4maker 번들)."""
    return _serve_html_with_nonce(request, abs_join(BASE_DIR, "static/vodstudio/index.html"))
