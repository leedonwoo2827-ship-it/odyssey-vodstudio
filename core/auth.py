...
class AuthManager:
    ...
    def create_oauth_user(
        ...
        *,
        ...
        """Create an account provisioned via an external identity provider
        (Google OAuth). Unlike create_user there is no caller-chosen password:
        we store a random unguessable hash so the password-login path can never
        authenticate this account — sign-in only happens through the OAuth flow.
        """
        username = username.strip().lower()
        if not username:
            return False
        if username in RESERVED_USERNAMES:
            logger.warning("Refused to create reserved OAuth username '%s'", username)
            return False
        if username in self.users:
            return False
        if "users" not in self._config:
        ...
        self._save()
        logger.info("Created OAuth user '%s' (provider=%s, admin=%s)", username, provider, is_admin)
        return True

    def create_session_for_user(self, username: str) -> Optional[str]:
        """Issue a session token for an already-authenticated user.

        Used by the OAuth callback: identity has been proven by the provider,
        so there is no password to verify. Mirrors create_session otherwise.
        """
        username = username.strip().lower()
