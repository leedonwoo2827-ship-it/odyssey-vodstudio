...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    # ------------------------------------------------------------------
    def _set_session_cookie(resp: Response, token: str, remember: bool) -> None:
        cookie_kwargs = dict(
            key=SESSION_COOKIE,
            value=token,
            httponly=True,
    ...
    async def google_login(request: Request, remember: bool = True):
        """Kick off the Google OAuth flow — redirect the browser to Google."""
        if not google_oauth.is_enabled():
            return RedirectResponse(url="/login?error=oauth_not_configured", status_code=302)
        if not _login_limiter.check(request.client.host):
            raise HTTPException(429, "Too many requests — try again later")
        url = google_oauth.begin_login(remember=remember)
    ...
    async def google_callback(
        ...
        if not code or not state:
            return RedirectResponse(url="/login?error=oauth_missing_code", status_code=302)
        try:
            info = await google_oauth.complete_login(code, state)
        except ValueError as e:
            logger.warning("Google OAuth failed: %s", e)
            return RedirectResponse(url="/login?error=oauth_failed", status_code=302)

        email = info["email"]
        # Hard gate: if an allowlist/domain is configured, the email must match.
        if google_oauth.allowlist_configured() and not google_oauth.email_matches_allowlist(email):
            logger.warning("Google OAuth: email not on allowlist: %s", email)
            return RedirectResponse(url="/login?error=oauth_not_allowed", status_code=302)

        username = email  # email doubles as the account key
        if username not in auth_manager.users:
            # Auto-provision only when it's clearly intended: first ever user
            # (becomes admin), an explicit allowlist match, or open signup.
            first_user = not auth_manager.is_configured
            may_create = (
                first_user
                or google_oauth.email_matches_allowlist(email)
                or auth_manager.signup_enabled
            )
            if not may_create:
                logger.warning("Google OAuth: no account for %s and auto-create not permitted", email)
                return RedirectResponse(url="/login?error=oauth_no_account", status_code=302)
            await asyncio.to_thread(
                auth_manager.create_oauth_user, username, email=email, is_admin=first_user
            )
        ...
        _set_session_cookie(redirect, token, bool(info.get("remember", True)))
        return redirect
    ...
    async def auth_status(request: Request):
        ...
        # Tell the login page whether to show the "Sign in with Google" button.
        result["google_oauth_enabled"] = google_oauth.is_enabled()
